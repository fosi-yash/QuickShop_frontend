...
const Address = ({ onselectedaddress }) => {
  ...
  return (
    <>
      <div>
        {
          address.map((user, item) => (
            <div className="container d-flex bg-white mt-2 rounded" key={user._id}>
              <div>
                <input className='mx-3 mt-3' style={{ height: '25px', width: '20px', }} value={user._id} checked={selectedAddress === user._id} onChange={(e) => { setSelectedAddress(e.target.value); onselectedaddress(e.target.value) }} type="radio" name="select" id="select" required />
              </div>
      ...
      <div className={`${display}`}>
        <div className="  ma-5 p-4 border rounded bg-light">
          ...
          <div className="ms-5 justify-content-around mt-5">
            <button className='d-flex  btn btn-primary' onClick={fetchLocation}>
          ...
          <form onSubmit={handleSubmit} >
            ...
            <div className="text-center mt-4">
              <button type="submit" className="btn btn-primary mx-2 px-4">Submit</button>
              <button type="button" onClick={() => { setDisplay('d-none'); setSelectedAddress() }} className="btn btn-danger mx-2 px-4">cancel</button>
            </div>
          </form>
        </div>
      </div>
